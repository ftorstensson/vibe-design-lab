...
import { 
  ...
  AlertCircle, HardHat, Layers, Network, Layout, Crown
} from 'lucide-react';
import { useVibeStore } from '@/store/vibe-store';
import { SpecialistAgent, VibeLayer } from '@/types/vibe-core';
import { clsx } from 'clsx';

// --- SECTION B: CONFIG & STRATEGIC ORDERING ---
const LAYERS_CONFIG: { id: VibeLayer; label: string; icon: any }[] = [
  { id: 'STRATEGY', label: 'Layer 1: Strategy', icon: Target },
  { id: 'JOURNEY', label: 'Layer 2: Journey', icon: Zap },
  { id: 'SITEMAP', label: 'Layer 3: Structure', icon: Network },
  { id: 'WIREFRAME', label: 'Layer 4: Wireframes', icon: Layout },
];

// THE MANDATORY STRATEGIC SEQUENCE
const DEPT_ORDER = [
  "BIG_IDEA_TEAM",
  "MARKET_TEAM",
  "AUDIENCE_TEAM",
...
export default function AgencyLab() {
  ...
  // --- SECTION C: HIERARCHY MAPPING (Layer -> Dept -> Agent) ---
  ...
  const handleSave = async () => {
    if (selectedAgent) {
        ...
        alert("Strategic DNA Committed to Firestore.");
    } else if (selectedDept) {
        await updateDeptInDB(selectedDept.id, { lens_profile: editorContent });
        alert("Department Lens Updated.");
    }
  };
  ...
  return (
    <div className="flex h-screen w-screen bg-[#fafafa] overflow-hidden font-sans text-slate-900">
      ...
      {/* --- SECTION D: THE SIDEBAR (TRIPLE ACCORDION) --- */}
      <div className="w-80 h-full bg-white border-r border-slate-200 flex flex-col shadow-2xl z-20 overflow-hidden text-slate-900">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white shrink-0">
            <button 
                ...
                className="p-2 hover:bg-slate-50 rounded-lg border border-transparent hover:border-slate-200 transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            {/* 1. THE GLOBAL COMMAND (Project Manager sits outside layers) */}
            <div className="space-y-2">
                ...
                {hierarchy['GLOBAL']?.['HUB']?.map((pm: any) => (
                    <button 
                        key={pm.id} 
                        onClick={() => handleSelectAgent(pm)} 
                        className={clsx(
                            "w-full p-4 border flex items-center gap-3 transition-all rounded-xl",
                            selectedAgent?.id === pm.id 
            ...
            {/* 2. THE ASSEMBLY LINE LAYERS */}
            <div className="space-y-3 pt-4 border-t border-slate-100">
                <span className="px-2 text-[9px] font-black text-slate-300 uppercase tracking-widest block mb-2 text-slate-900">The Assembly Line</span>
                                    ...
                                    hierarchy[layer.id]?.[deptId] && (
                                        <div key={deptId} className="space-y-1">
                                            <button 
                                                ...
                                                className={clsx(
                                                    "w-full p-2 flex items-center justify-between rounded-lg transition-colors group",
                                                    selectedDept?.id === deptId ? "bg-slate-100 text-slate-900" : "text-slate-400 hover:text-slate-600"
                                                )}
                                            >
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] font-black uppercase tracking-widest">{deptId.replace(/_TEAM/g, '').replace(/_/g, ' ')}</span>
                                                </div>
                                                ...
                                                <div className="pl-4 space-y-1">
                                                    {hierarchy[layer.id][deptId].sort((a, b) => a.role_index - b.role_index).map((agent) => (
                                                        <button 
                                                            ...
                                                            className={clsx(
                                                                "w-full text-left p-2 rounded-lg text-xs font-bold transition-all border",
                                                                selectedAgent?.id === agent.id 
                                                                    ? "bg-white border-slate-900 text-slate-900 shadow-sm translate-x-1" 
      ...
      {/* --- SECTION E: THE ADVERSARIAL EDITOR --- */}
      <div className="flex-1 h-full flex flex-col bg-[#fdfdfd] overflow-hidden">
        {(selectedAgent || selectedDept) ? (
                ...
                <header className="flex justify-between items-end mb-12 shrink-0">
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            <span className="bg-slate-900 text-white text-[9px] font-black px-2 py-1 rounded tracking-widest uppercase">
                                {selectedAgent ? selectedAgent.model_tier : "DEPT LENS"}
                            </span>
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">
                                {selectedAgent ? selectedAgent.layer_id : selectedLayer} // {selectedAgent ? selectedAgent.dept_id : selectedDept.id}
                            </span>
                        </div>
                        <h1 className="font-playfair text-6xl font-black text-slate-900 italic tracking-tighter leading-none">
                            {selectedAgent ? selectedAgent.display_name : selectedDept.label}
                        </h1>
                    </div>
                    <button onClick={handleSave} className="flex items-center gap-4 bg-slate-900 text-white px-10 py-5 rounded-2xl shadow-2xl hover:bg-black transition-all active:scale-95 shrink-0">
                        <Save className="w-5 h-5" />
                        <span className="font-bold text-sm uppercase tracking-widest">Commit Changes</span>
                    </button>
                </header>
                ...
                <div className="grid grid-cols-3 gap-8 items-start pb-20">
                    ...
                    <div className="col-span-2 flex flex-col bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden ring-1 ring-black/5 min-h-[600px]">
                        <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">
                                {selectedAgent ? "Cognitive Persona Prompt" : "Departmental Worldview Lens"}
                            </span>
                        </div>
                        {/* FIX: Added title and aria-label to satisfy accessibility linter */}
                        <textarea 
                            title="Cognitive Persona Prompt"
                            aria-label="Cognitive Persona Prompt"
                            className="flex-1 p-10 text-lg font-medium text-slate-700 leading-relaxed outline-none resize-none bg-transparent" 
                            value={editorContent} 
                            onChange={(e) => setEditorContent(e.target.value)} 
                            placeholder="Engineering the brain..."
                        />
                    </div>

                    {/* ADVERSARIAL METADATA (DeTax Constraints) */}
                        ...
                        {selectedAgent && (
                            <>
                                <MetadataBlock 
                                    ...
                                    value={optTarget} 
                                    onChange={setOptTarget}
                                    placeholder="Maximize venture moat..." 
                                />
                                <MetadataBlock 
                                    icon={AlertCircle} 
                                    label="Loss Function (Failure)" 
                                    value={lossFunc} 
                                    onChange={setLossFunc}
                                    color="text-red-500" 
                                    placeholder="Punish for incrementalism..." 
                                />
                                <MetadataBlock 
                                    icon={HardHat} 
                                    ...
                                    value={physics} 
                                    onChange={setPhysics}
                                    placeholder="Runway is 9 months..." 
                                />
                            </>
                        )}
                        
                        <div className="p-8 bg-slate-100 rounded-[2.5rem] border border-slate-200">
                            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-4 text-slate-900">Technical Specs</h3>
                            <div className="space-y-3">
                                <SpecRow label="Type" value={selectedAgent ? "SPECIALIST" : "LENS"} />
                                <SpecRow label="ID" value={selectedAgent ? selectedAgent.id : selectedDept.id} />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex-1 flex items-center justify-center text-slate-300 flex-col gap-4 opacity-30 animate-pulse">
                <HardHat className="w-12 h-12" />
                <p className="text-[11px] font-black uppercase tracking-[0.4em]">Select a unit to engineer</p>
            </div>
        )}
      </div>
    </div>
...
const MetadataBlock = ({ icon: Icon, label, value, onChange, placeholder, color = "text-slate-400" }: any) => (
    <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm hover:border-slate-300 transition-colors text-slate-900">
        ...
        <textarea 
            title={label}
            aria-label={label}
            className="w-full text-xs font-bold text-slate-800 bg-transparent border-none outline-none resize-none h-24 p-0 focus:ring-0 placeholder:text-slate-200" 
            value={value} 
...
const SpecRow = ({ label, value }: { label: string; value: string }) => (
    <div className="flex justify-between items-center text-[10px] text-slate-900">
        <span className="font-bold text-slate-400 uppercase tracking-widest">{label}</span>
        <span className="font-black text-slate-900 truncate ml-4 uppercase">{value}</span>
    </div>
);
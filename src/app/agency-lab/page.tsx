...
import { 
  ...
  AlertCircle, HardHat, Layers, Network, Layout, Crown, Search, PenTool
} from 'lucide-react';
      ...
      <div className="w-80 h-full bg-white border-r border-slate-200 flex flex-col shadow-2xl z-20 overflow-hidden text-slate-900">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="space-y-2">
                ...
                {hierarchy['GLOBAL']?.['HUB']?.sort((a, b) => a.role_index - b.role_index).map((agent: SpecialistAgent) => (
                    <button 
                        key={agent.id} 
                        onClick={() => handleSelectAgent(agent)} 
                        className={clsx(
                            "w-full p-4 border flex items-center gap-3 transition-all rounded-xl",
                            selectedAgent?.id === agent.id ? "border-slate-900 bg-slate-900 text-white shadow-lg" : "bg-white border-slate-100 hover:border-slate-300 text-slate-600"
                        )}
                    >
                        {agent.id.includes('editor') ? <PenTool className="w-4 h-4" /> : <Crown className="w-4 h-4" />}
                        <span className="text-xs font-black uppercase tracking-widest">{agent.display_name}</span>
                    </button>
                ))}
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-100">
                ...
                {LAYERS_CONFIG.map((layer) => (
                    <div key={layer.id} className="space-y-1">
                        <button onClick={() => setSelectedLayer(layer.id)} className={clsx(
            ...
            <div className="flex-1 flex flex-col p-12 max-w-7xl mx-auto w-full overflow-y-auto scrollbar-hide">
                ...
                <div className="grid grid-cols-3 gap-8 items-start pb-20">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-8 bg-slate-100 rounded-[2.5rem] border border-slate-200"><h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-4 text-slate-900">Technical Specs</h3><div className="space-y-3"><SpecRow label="Type" value={selectedAgent ? "SPECIALIST" : "LENS"} /><SpecRow label="ID" value={selectedAgent ? selectedAgent.id : selectedDept.id} /><SpecRow label="Index" value={(selectedAgent?.role_index || '0').toString()} /></div></div>
                    </div>
                </div>
            </div>
        ) : (
